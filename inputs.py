SOURCE_TABLES = [
    "BALANCE-PAIEMENTS",
    "CHOMAGE-TRIM-NATIONAL",
    "CLIMAT-AFFAIRES",
    "CNA-2020-CONSO-MEN",
    "CNA-2020-CONSO-SI",
    "CNA-2020-CPEB",
    "CNA-2020-CSI",
    "CNA-2020-EMPLOI",
    "CNA-2020-ERE",
    "CNA-2020-FBCF-SI",
    "CNA-2020-PIB",
    "CNA-2020-TEI",
    "CNT-2020-CB",
    "CNT-2020-CSI",
    "CNT-2020-OPERATIONS",
    "CNT-2020-PIB-EQB-RF",
    "COM-EXT",
    "COMPTES-ETAT",
    "CONSO-MENAGES-2020",
    "CONSTRUCTION-LOCAUX",
    "CONSTRUCTION-LOGEMENTS",
    "CREATIONS-ENTREPRISES",
    "CREATIONS-ENTREPRISES-METHODE-2022",
    "DECES-MORTALITE",
    "DEFAILLANCES-ENTREPRISES",
    "DEMANDES-EMPLOIS-NATIONALES",
    "DETTE-NEGOCIABLE-ETAT",
    "DETTE-TRIM-APU-2020",
    "EMPLOI-BIT-TRIM",
    "EMPLOI-SALARIE-TRIM-NATIONAL",
    "ENQ-CONJ-ACT-IND",
    "ENQ-CONJ-ART-BAT",
    "ENQ-CONJ-COM-DET",
    "ENQ-CONJ-COM-GROS",
    "ENQ-CONJ-IND-BAT",
    "ENQ-CONJ-INV-IND",
    "ENQ-CONJ-MENAGES",
    "ENQ-CONJ-PROMO-IMMO",
    "ENQ-CONJ-SERV",
    "ENQ-CONJ-TP",
    "ENQ-CONJ-TRES-IND",
    "ERI-ACTIVITE-PARTIELLE",
    "IC-PROD-CONS-2021",
    "ICA-2021-COMMERCE",
    "ICA-2021-IND-CONS",
    "ICA-2021-SERVICES",
    "ICT-2020",
    "INDICE-TRAITEMENT-FP",
    "INDICES_LOYERS",
    "IP-PROD-CONS-N-HAB-2021",
    "IPAGRI-BASE-2020",
    "IPC-PM-2015",
    "IPCH-2015",
    "IPEA-2021",
    "IPGD-2015",
    "IPI-1990",
    "IPI-2021",
    "IPLA-IPLNA-2015",
    "IPPI-2021",
    "IPPMP-NF",
    "IPPS-2021",
    "IPS-2021-SERVICES",
    "IRL",
    "NAISSANCES-FECONDITE",
    "PARC-LOGEMENTS",
    "POPULATION-STRUCTURE",
    "SALAIRES-ACEMO",
    "SALAIRES-ACEMO-2017",
    "SALAIRES-ANNUELS",
    "SMIC-COTISATIONS",
    "SUIVI-DEMANDE-TOURISTIQUE",
    "TAUX-CHOMAGE",
    "TCRED-CONDITIONSDEVIE-LOG-SOC",
    "TCRED-CONDITIONSDEVIE-TXP-CDE",
    "TCRED-EDUCATION-APP-JEN",
    "TCRED-EDUCATION-DIPLOMES-TECHNIQUES",
    "TCRED-EDUCATION-EFF-PSS",
    "TCRED-EDUCATION-ENS-PSD",
    "TCRED-EDUCATION-ETAB-SCOL",
    "TCRED-EDUCATION-REU-BAC",
    "TCRED-EMPLOI-31-DECEMBRE-BASE-2018",
    "TCRED-EMPLOI-SALARIE-TRIM",
    "TCRED-ENTREPRISES-EFF-OPE",
    "TCRED-ENTREPRISES-EMP-SAL-AN-TAILLE",
    "TCRED-ESTIMATIONS-POPULATION",
    "TCRED-SALAIRES-REVENUS-MEN",
    "TCRED-SALAIRES-REVENUS-RET-BEN",
    "TCRED-SALAIRES-REVENUS-REV-SAL-SEXE-CS",
    "TCRED-SALAIRES-REVENUS-TAUX-PAUVRETE-AGE",
    "TCRED-SALAIRES-REVENUS-TNB",
    "TCRED-SANTE-ACCUEIL-PERS-AGEES",
    "TCRED-SANTE-PERSONNELS",
    "TCRED-TRAVAIL-EMPLOI-EFF-FP-RATIO",
    "TCRED-TRAVAIL-EMPLOI-EFF-FPE",
    "TCRED-TRAVAIL-EMPLOI-EFF-FPH",
    "TCRED-TRAVAIL-EMPLOI-EFF-FPT",
    "TCRED-TRAVAIL-EMPLOI-TCHOMA-SA",
    "TOURISME-FRANCE-METHODE-REDRESSEMENT-2019",
    "TRANSPORTS",
    "VOV-2021-COMMERCE"
]